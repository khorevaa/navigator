...
Процедура КомандаЗаполнитьЖурналРегистрацииПоОбъекту(Команда)
	...
	ВыгрузитьЖурналРегистрацииПоОбъекту();
...
&НаСервере
Процедура ВыгрузитьЖурналРегистрацииПоОбъекту()
	
	Если ДоступноРасширениеНавигатора() Тогда
		ВыгрузитьЖурналРегистрацииПоОбъектуАсинхронно();
	Иначе
		ВыгрузитьЖурналРегистрацииПоОбъектуПоследовательно();
	КонецЕсли;
	
КонецПроцедуры

&НаСервере
Процедура ВыгрузитьЖурналРегистрацииПоОбъектуПоследовательно()
...
&НаСервере
Процедура ВыгрузитьЖурналРегистрацииПоОбъектуАсинхронно()
	
	ПараметрыФЗ = Новый Массив;
	ПараметрыФЗ.Добавить(ОбъектБазы.Ссылка);
	
	тз_ЗаписиЖурналаРегистрации = РеквизитФормыВЗначение("ЗаписиЖурналаРегистрации");
	
	ПараметрыФЗ.Добавить(тз_ЗаписиЖурналаРегистрации);
	
	ОписаниеЗадания = "(НК) ВыгрузитьЖурналРегистрацииПоОбъекту : " + ОбъектБазы.Ссылка;
	
	ФоновыеЗадания.Выполнить("НК_НавигаторСервер.ВыгрузитьЖурналРегистрацииПоОбъекту", 
		ПараметрыФЗ, 
...
&НаСервере
Функция ДоступноРасширениеНавигатора()
	
	Возврат Не (Метаданные.ОбщиеМодули.Найти("НК_НавигаторСервер") = Неопределено);
